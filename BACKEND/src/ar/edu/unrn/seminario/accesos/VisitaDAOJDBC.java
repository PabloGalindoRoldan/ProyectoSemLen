...
public class VisitaDAOJDBC implements VisitaDAO {
        ...
        try {
            try (Connection conn = DBConnection.getConnection()) {
                ...
                try (PreparedStatement ps = conn.prepareStatement(INSERT_VISITA, Statement.RETURN_GENERATED_KEYS)) {
                    ...
                    if (visita.getOrdenRetiro() == null || visita.getOrdenRetiro().getIdOrdenes() == null) ps.setNull(7, java.sql.Types.INTEGER); else ps.setInt(7, visita.getOrdenRetiro().getIdOrdenes());
                    ...
                    // if visita final, update orden status
            ...
            logger.log(Level.SEVERE, "Error creating visita", ex);
            throw new PersistenceException("Error creating visita", ex);
        }
    }
    ...
    public List<Visita> findAll() throws SQLException {
        try {
            ...
            logger.log(Level.SEVERE, "Error finding visitas", e);
            throw new PersistenceException("Error finding visitas", e);
        }
    }
    ...
    public Visita findById(int id) throws SQLException {
        try {
            ...
            logger.log(Level.SEVERE, "Error finding visita by id=" + id, e);
            throw new PersistenceException("Error finding visita by id=" + id, e);
        }
    }
    ...
    public List<Visita> listarPorOrden(int ordenId) throws SQLException {
        try {
            List<Visita> result = new ArrayList<>();
            try (Connection conn = DBConnection.getConnection(); PreparedStatement ps = conn.prepareStatement(SELECT_VISITAS_BY_ORDEN)) {
                ps.setInt(1, ordenId);
                try (ResultSet rs = ps.executeQuery()) {
                    while (rs.next()) {
                        Visita v = mapVisita(rs);
            ...
            logger.log(Level.SEVERE, "Error listing visitas by orden=" + ordenId, e);
            throw new PersistenceException("Error listing visitas by orden=" + ordenId, e);
        }
    }
    ...
    public void cancelar(int id) throws SQLException {
        try {
            try (Connection conn = DBConnection.getConnection(); PreparedStatement ps = conn.prepareStatement(UPDATE_CANCEL)) {
                ps.setInt(1, id);
                ps.executeUpdate();
            }
        } catch (SQLException e) {
            logger.log(Level.SEVERE, "Error cancelling visita id=" + id, e);
            throw new PersistenceException("Error cancelling visita id=" + id, e);
        }
    }
    ...
    public void delete(int id) throws SQLException {
        try {
            ...
            logger.log(Level.SEVERE, "Error deleting visita id=" + id, e);
            throw new PersistenceException("Error deleting visita id=" + id, e);
        }
    }
    ...
    private Visita mapVisita(ResultSet rs) throws SQLException {
        ...
        if (ordenId == null) {
            ...
            // create a placeholder OrdenRetiro with id only
